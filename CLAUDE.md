...
- **AI 분석 리포트가 지역 때문에 자꾸 "부적합"으로 나오던 버그, 2026-09-05
  수정**: 사용자 지적("지역 매핑이 안 되고 있나? K패스도 지역으로 부적합이라고
  나와") — K패스처럼 진짜 전국 상품도 `region_code`엔 법정동코드가 187개나
  콤마로 나열돼 있는데(온통청년 원본 API를 그대로 캐시), `_policy_text()`가
  이걸 "지역 코드: 11110,11140,..."로 그대로 프롬프트에 덤프하고 있었다 — LLM이
  이 숫자 뭉치를 해석 못 하고 시스템 프롬프트의 "애매하면 보수적으로 부적합"
  지침을 따라 지역 때문에 부적합 처리해버린 것(marital_status 원본 코드값을
  프롬프트에서 뺀 것과 같은 종류의 버그인데, region_code에는 그 처리가 안
  돼있었다). `_region_summary()`를 추가해 `region_names_for_prefix()`로 사람이
  읽을 수 있는 지역명 목록으로 바꾸고(예: "지역 조건: 서울, 부산"), 15개 이상
  시/도를 커버하면 "전국 (지역 제한 없음)"으로 요약한다.
- **위 지역 버그 발견 후 사용자가 "다른 부분들도 제대로 들어가고 있는지
  확인해봐라"고 요청해서 전수 감사(2026-09-05)한 결과, 2개 더 나옴**:
  (1) `policy_chat/router.py`의 `_describe_filters()`(정책달력 챗봇이 "지금
  적용된 조건"을 스스로에게 설명할 때 씀)에 `occupation`/`is_sme_employee`가
  빠져 있었다 — 실제 검색 결과 필터링(`_matches()`)엔 반영되는데 챗봇이 그
  조건을 사용자에게 설명은 못 하는 불일치. (2) `policy_chat/analysis.py`의
  `_profile_text()`(AI 분석 리포트와 정책별 챗봇이 공유)에 occupation/
  is_sme_employee/has_disability/is_veteran이 아예 없었다 — matching.py의
  TARGETING_RULES가 이 필드들로 정책을 거르는데, 개별 정책 리포트는 이 정보를
  모른 채 나이/소득/혼인/지역만으로 적합 여부를 판단하고 있었다(재직자 전용
  정책을 학생이 봐도 "적합"으로 오판할 수 있는 정보 공백). 둘 다 값 채워서
  고쳤고, `OccupationType` 한국어 라벨(`OCCUPATION_LABELS`)은 두 파일이 각자
  복붙하지 않도록 `matching.py`에 모아뒀다 — `frontend/lib/profileOptions.ts`의
  `OCCUPATION_OPTIONS` 라벨과 반드시 같은 값으로 유지할 것.
