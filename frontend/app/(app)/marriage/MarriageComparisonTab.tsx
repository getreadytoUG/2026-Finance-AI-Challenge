...
import { AlertTriangle, Home, Info, TrendingDown, TrendingUp } from "lucide-react";
import { compareMarriageScenarios, getMe, type HousingLoanMarriageComparison, type MarriageComparisonOutput } from "@/lib/api";
...
function HousingComparisonCard({ comparison }: { comparison: HousingLoanMarriageComparison }) {
  ...
  const interestDelta = married.monthly_interest_krw - unmarried.monthly_interest_krw;
    ...
    <div className="grid gap-5 xl:grid-cols-[1.4fr_.8fr]">
      <section className="relative overflow-hidden rounded-[24px] bg-[#0d1b36] p-7 text-white sm:p-9">
        ...
        <div className="relative">
          ...
          <div className="mt-4 grid grid-cols-2 gap-4">
            <div>
              <div className="text-[10px] font-bold text-blue-100/60">미혼 · {unmarried.product_name}</div>
              <div className="mt-2 text-[28px] font-extrabold tracking-[-.04em] sm:text-[32px]">
                {unmarried.eligible ? `연 ${(unmarried.policy_rate * 100).toFixed(2)}%` : "대상 아님"}
              </div>
            </div>
            <div>
              <div className="text-[10px] font-bold text-[#9cc5ff]">기혼 · {married.product_name}</div>
              <div className="mt-2 text-[28px] font-extrabold tracking-[-.04em] text-[#9cc5ff] sm:text-[32px]">
                {married.eligible ? `연 ${(married.policy_rate * 100).toFixed(2)}%` : "대상 아님"}
              </div>
            </div>
          </div>
          <div className="mt-6 grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-white/10 p-3.5">
              <div className="text-[10px] font-bold text-blue-100/60">대출 가능액 · 월 이자</div>
              <div className="mt-2 text-[14px] font-extrabold">
                {unmarried.eligible ? `${unmarried.loan_amount_krw.toLocaleString()}원 · 월 ${unmarried.monthly_interest_krw.toLocaleString()}원` : "-"}
              </div>
            </div>
            <div className="rounded-xl bg-white/10 p-3.5">
              <div className="text-[10px] font-bold text-blue-100/60">대출 가능액 · 월 이자</div>
              <div className="mt-2 text-[14px] font-extrabold">
                {married.eligible ? `${married.loan_amount_krw.toLocaleString()}원 · 월 ${married.monthly_interest_krw.toLocaleString()}원` : "-"}
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="rounded-[24px] border border-slate-200/80 bg-white p-6">
        <div className="flex items-center gap-3">
          <span className={`grid h-9 w-9 place-items-center rounded-xl ${interestDelta > 0 ? "bg-[#fdeeee] text-rose-500" : "bg-[#e6f8f5] text-[#159c8d]"}`}>
            {interestDelta > 0 ? <TrendingUp size={17} /> : <TrendingDown size={17} />}
          </span>
          <div>
            <div className="text-[10px] font-extrabold uppercase tracking-[.18em] text-slate-400">결혼하면 뭐가 달라지나</div>
            <h2 className="mt-1 text-[15px] font-extrabold tracking-[-.03em]">혼인 후 변화</h2>
          </div>
        </div>
        {unmarried.eligible && married.eligible ? (
          <>
            <div className="mt-5 rounded-xl bg-[#f7f9fc] px-4 py-3.5">
              <div className="text-[11px] font-bold text-slate-500">금리 변화</div>
              <div className={`mt-1 text-[18px] font-extrabold ${rateDeltaPct <= 0 ? "text-[#159c8d]" : "text-rose-500"}`}>
                {rateDeltaPct > 0 ? "+" : ""}
                {rateDeltaPct.toFixed(2)}%p
              </div>
            </div>
            <div className="mt-3 rounded-xl bg-[#f7f9fc] px-4 py-3.5">
              <div className="text-[11px] font-bold text-slate-500">월 이자 변화</div>
              <div className={`mt-1 text-[18px] font-extrabold ${interestDelta <= 0 ? "text-[#159c8d]" : "text-rose-500"}`}>
                {interestDelta > 0 ? "+" : ""}
        ...
        <p className="mt-4 text-[11px] leading-5 text-slate-500">{married.eligible ? married.summary : unmarried.summary}</p>
      </section>
    </div>
  );
}
      ...
      <div className="mb-6 flex items-start gap-3 rounded-2xl border border-slate-200/80 bg-white p-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[12px] leading-5 text-slate-500">
            ...
            매매는 <b>[미혼] 내집마련 디딤돌대출</b> vs <b>[기혼] 신혼부부전용 디딤돌대출</b>로 금리·대출한도·
            월 이자를 나란히 보여드려요.
          </p>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-6 grid gap-6">
          <div className="flex items-start gap-2 rounded-xl bg-[#fff7e6] p-3.5 text-[12px] font-bold leading-5 text-[#946200]">
            ...
            금리·소득상한·대출한도는 2026년 8월 고시 기준 실제 수치예요. 생애최초 우대, 지방 주택 인하 등
            일부 조건은 반영하지 못했어요 — 정확한 조건은 주택도시기금 공고를 확인하세요.
          </div>
