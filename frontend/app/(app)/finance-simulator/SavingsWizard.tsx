...
import {
  getMe,
  simulateYouthFutureSavings,
  type YouthFutureSavingsOutput,
} from "@/lib/api";
...
const REAL_DISCLAIMER =
  ...
  "시중적금 비교 금리는 은행마다 달라 가정치예요\n정확한 조건은 서민금융진흥원 공고를 확인하세요.";
...
function mockResult(
  ...
  const interest = Math.round(
    ((monthly * (n * (n + 1))) / 2) * (p.savingsRate / 12),
  );
  return {
    preferential,
    matchRate,
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div>
              ...
              {product.real && (
                <p className="mt-2 text-[11px] font-semibold text-slate-400">
                  청년미래적금은 저장된 프로필의 재직 여부를 기준으로 실제
                  계산돼요
                  <br /> 이 토글은 예시 상품(청년도약계좌/청년주택드림)에만
                  적용돼요.
                </p>
              )}
            </div>
            <div>
              ...
              <div className="flex h-12 items-center rounded-xl bg-[#eef3f9] px-4 text-[13px] font-extrabold text-slate-500">
                {product.termMonths % 12 === 0
                  ? `${product.termMonths / 12}년 (고정)`
                  : `${product.termMonths}개월 (고정)`}
              </div>
            </div>
          </div>

          <div className="mt-7 rounded-2xl border border-slate-200 bg-[#f7f9fc] px-5 py-2">
            <div className="py-2 text-[11px] font-extrabold uppercase tracking-[.16em] text-slate-400">
              자격 예비판정
            </div>
            ...
            ? realResult && (
                <div>
                  <div className="rounded-2xl bg-[#0d1b36] p-6 text-white sm:p-7">
                      ...
                      {realResult.eligible ? (
                        <>
                          ...
                          <span className="text-[#9cc5ff]">
                            {manwon(krwToManwon(realResult.policy_total_krw))}
                          </span>
                          을 받을 수 있어요.
                        </>
                      ) : (
                    ...
                    <p className="mt-3 text-[12px] leading-6 text-blue-100/70">
                      {realResult.eligibility_note}
                    </p>
                    <div className="mt-6 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-xl bg-white/10 p-3.5">
                        ...
                        <div className="mt-2 text-[15px] font-extrabold">
                          {realResult.policy_total_krw.toLocaleString()}원
                        </div>
                      </div>
                      <div className="rounded-xl bg-white/10 p-3.5">
                        <div className="text-[10px] font-bold text-blue-100/60">
                        ...
                        <div className="mt-2 text-[15px] font-extrabold">
                          {realResult.market_total_krw.toLocaleString()}원
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-5">
                    ...
                    <div className="mt-4 rounded-xl bg-[#f7f9fc] px-4 py-3.5">
                      ...
                      <div
                        className={`mt-1 text-[18px] font-extrabold ${realResult.benefit_diff_krw > 0 ? "text-[#159c8d]" : "text-slate-500"}`}
                      >
                      ...
                      <div className="mt-4 grid gap-3">
                        {realResult.matched_policies.map((p) => (
                          ...
                          >
                            ...
                            <div className="mt-1.5 text-[11px] font-semibold text-slate-400">
                              신청 기간{" "}
                              {formatApplicationPeriod(p.application_period)}
                            </div>
                            <PolicyDetailLink
                              url={p.reference_url}
                              className="mt-1.5"
                ...
                return (
                  <div>
                    <div className="rounded-2xl bg-[#f4f8f4] p-6">
                      ...
                      <div className="mt-2 text-[40px] font-extrabold tracking-[-.05em] text-[#2f7a3f]">
                        {manwon(r.maturity)}
                      </div>
