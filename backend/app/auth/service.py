...
def _backfill_if_empty(user: User, *, name: str | None) -> bool:
    # 소셜에서 받은 이름은 사용자가 직접 입력한 값이 없을 때만 채운다
...
def get_or_create_social_user(
    ...
    *,
    ...
    if existing is not None:
        if _backfill_if_empty(existing, name=name):
            db.commit()
            db.refresh(existing)
    ...
    if email:
        ...
        if by_email is not None:
            ...
            _backfill_if_empty(by_email, name=name)
            db.commit()
            db.refresh(by_email)
