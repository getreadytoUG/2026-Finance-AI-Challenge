...
def _simulate_jeonse(input: HousingLoanInput, *, is_married: bool | None, age: int | None) -> HousingLoanOutput:
  ...
  # 미혼(청년전용)은 본인 소득, 기혼(신혼부부전용)은 부부합산 소득 기준이라
  # 문구도 맞춰준다 — 예전엔 미혼인데도 "부부합산"이라고 나왔다(2026-09-06 사용자 지적).
  income_label = "부부합산 연소득" if is_married else "본인 연소득"

  if not age_ok:
    summary = f"{product_name}은 만 {_JEONSE_AGE_MIN}~{_JEONSE_AGE_MAX}세만 신청할 수 있어요."
  elif not eligible:
    summary = f"{income_label}이 기준({income_cap:,}원)을 초과해 이 상품 대상이 아니에요."
  else:
    summary = (
...
# 생애최초 주택구입자는 일반(6,000만원)보다 높은 7,000만원까지 허용된다 — 이 앱은
# "생애최초 여부" 입력이 없어 계산엔 반영 못 하지만, 소득이 일반 상한과 이 값
# 사이일 때 결과 문구로 안내한다(2026-09-06 사용자 지적: 미혼 6,200만원이
# "대상 아님"으로 나오는데 생애최초면 될 수도 있다).
_PURCHASE_INCOME_CAP_FIRST_HOME_KRW = 70_000_000
...
def _simulate_purchase(input: HousingLoanInput, *, is_married: bool | None) -> HousingLoanOutput:
  ...
  # 미혼(내집마련)은 본인 소득, 기혼(신혼부부전용)은 부부합산 소득 기준.
  income_label = "부부합산 연소득" if newlywed else "본인 연소득"
  ...
  if eligible:
    ...
    summary = f"{income_label}이 기준({income_cap:,}원)을 초과해 이 상품 대상이 아니에요."
    if not newlywed and income <= _PURCHASE_INCOME_CAP_FIRST_HOME_KRW:
      summary += (
          f" 단, 생애최초 주택구입자는 {_PURCHASE_INCOME_CAP_FIRST_HOME_KRW:,}원까지 가능해요.\n"
          "이 계산기는 생애최초 여부를 반영하지 않아요."
      )
